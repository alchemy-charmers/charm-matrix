"""Helper library for testing charms on real Juju controllers."""
import base64
import pickle
...
class JujuTools:
    """Helper lib for interacting with Juju."""
    ...
    async def remote_object(self, imports, remote_cmd, target):
        """
        Run a command on target machine and returns a python object of the result.
    ...
    async def convert_config(self, config):
        """Convert the config dictionary from get_config to one valid for set_config."""
        clean_config = {}
